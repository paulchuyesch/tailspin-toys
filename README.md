...
The repository's coding standards are maintained in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) and the focused [instruction files](.github/instructions/). In particular:

- Comments explain intent and decisions, not code that is already self-explanatory.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc to document their purpose, parameters, and return values.
- Reusable Astro components document their `Props` interfaces and public contracts.
- TypeScript uses two-space indentation, single quotes, semicolons, and trailing commas in multiline expressions; ESLint checks these conventions.
